...
BB.register("ZONE_CHANGED",
	function()  -- Delay by 3 seconds so UnitXP can return the correct value
		BB.wait(3,
			function()
	...
	function()
		...
		BB.wait(3,
			function()
				...
				local combat_time_pct = BB.db.total_combat_time / BB.db.total_time * 100
				local prev_lvl_combat_time_pct = BB.db.combat_time_per_lvl[prev_lvl] / BB.db.time_per_lvl[prev_lvl] * 100

				BB.print_addon_msg("")
				print(
					"Time spent in previous level: " ..
					BB.highlight(BB.display_time(BB.db.prev_lvl_time))
				)
				print(
					"So far, you have spent " ..
					BB.highlight(
						string.format("%.1f", combat_time_pct)
					) ..
					"% of your time in combat! (" ..
					BB.highlight(
						string.format("%.1f", prev_lvl_combat_time_pct)
					) ..
